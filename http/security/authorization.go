...
func AuthorizedTeamManagement(teamID baasapi.TeamID, context *RestrictedRequestContext) bool {
	...
	for _, membership := range context.UserMemberships {
		if membership.TeamID == teamID && membership.Role == baasapi.TeamLeader {
			return true
		}
	}
...
// authorizedBaask8sAccess ensure that the user can access the specified baask8s.
// It will check if the user is part of the authorized users or part of a team that is
// listed in the authorized teams of the baask8s and the associated group.
//func authorizedBaask8sAccess(baask8s *baasapi.Baask8s, baask8sGroup *baasapi.Baask8sGroup, userID baasapi.UserID, memberships []baasapi.TeamMembership) bool {
//	groupAccess := authorizedAccess(userID, memberships, baask8sGroup.AuthorizedUsers, baask8sGroup.AuthorizedTeams)
//	if !groupAccess {
//		return authorizedAccess(userID, memberships, baask8s.AuthorizedUsers, baask8s.AuthorizedTeams)
//	}
//	return true
//}
...
// It will check if the user is part of the authorized users or part of a team that is
// listed in the authorized teams of the baask8s and the associated group.
func authorizedBaask8sAccess(baask8s *baasapi.Baask8s, userID baasapi.UserID, memberships []baasapi.TeamMembership) bool {
	//groupAccess := authorizedAccess(userID, memberships, baask8sGroup.AuthorizedUsers, baask8sGroup.AuthorizedTeams)
	//if !groupAccess {
		return authorizedAccess(userID, memberships, baask8s.AuthorizedUsers, baask8s.AuthorizedTeams)
	//}
...
// authorizedBaask8sGroupAccess ensure that the user can access the specified baask8s group.
// It will check if the user is part of the authorized users or part of a team that is
// listed in the authorized teams.
func authorizedBaask8sGroupAccess(baask8sGroup *baasapi.Baask8sGroup, userID baasapi.UserID, memberships []baasapi.TeamMembership) bool {
	return authorizedAccess(userID, memberships, baask8sGroup.AuthorizedUsers, baask8sGroup.AuthorizedTeams)
}
...
func authorizedAccess(userID baasapi.UserID, memberships []baasapi.TeamMembership, authorizedUsers []baasapi.UserID, authorizedTeams []baasapi.TeamID) bool {
