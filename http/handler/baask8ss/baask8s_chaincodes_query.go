...
type queryccauthenticatePayload struct {
...
func (payload *queryccauthenticatePayload) Validate(r *http.Request) error {
		return nil;
}


// GET request on /api/baask8ss
func (handler *Handler) baask8sChaincodesQuery(w http.ResponseWriter, r *http.Request) *httperror.HandlerError {
	...
	var payload queryccauthenticatePayload
	...
	if err == baasapi.ErrObjectNotFound {
		...
		req, err := http.NewRequest("POST", sdk_url+"/channels/"+channelname+"/chaincodes/"+payload.ChaincodeName+"/query", bytes.NewBuffer(jsonValue))
