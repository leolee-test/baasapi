...
func (payload *templateCreatePayload) Validate(r *http.Request) error {
	if payload.Type == 0 || (payload.Type != 1 && payload.Type != 2 && payload.Type != 3) {
		return baasapi.Error("Invalid template type. Valid values are: 1 (container), 2 (Swarm stack template) or 3 (Compose stack template).")
	}
	if govalidator.IsNull(payload.Title) {
		return baasapi.Error("Invalid template title")
	}
	if govalidator.IsNull(payload.Description) {
		return baasapi.Error("Invalid template description")
	}

	if payload.Type == 1 {
		if govalidator.IsNull(payload.Image) {
			return baasapi.Error("Invalid template image")
		}
	}

	if payload.Type == 2 || payload.Type == 3 {
		if govalidator.IsNull(payload.Repository.URL) {
			return baasapi.Error("Invalid template repository URL")
		}
...
func (handler *Handler) templateCreate(w http.ResponseWriter, r *http.Request) *httperror.HandlerError {
	...
	template := &baasapi.Template{
		Type:              baasapi.TemplateType(payload.Type),
		Title:             payload.Title,
	...
	if template.Type == baasapi.SwarmStackTemplate || template.Type == baasapi.ComposeStackTemplate {
