...
func (handler *Handler) proxyRequestsToRegistryAPI(w http.ResponseWriter, r *http.Request) *httperror.HandlerError {
	...
	if err != nil {
		return &httperror.HandlerError{http.StatusForbidden, "Permission denied to access registry", baasapi.ErrEndpointAccessDenied}
	}

	extension, err := handler.ExtensionService.Extension(baasapi.RegistryManagementExtension)
	if err == baasapi.ErrObjectNotFound {
	...
	proxy = handler.ProxyManager.GetExtensionProxy(baasapi.RegistryManagementExtension)
	if proxy == nil {
		proxy, err = handler.ProxyManager.CreateExtensionProxy(baasapi.RegistryManagementExtension)
		if err != nil {
	...
	r.Header.Set("X-BaaSapiExtension-License", extension.License.LicenseKey)

	http.StripPrefix("/registries/"+id, proxy).ServeHTTP(w, r)
	return nil
}

func createDefaultManagementConfiguration(registry *baasapi.Registry) *baasapi.RegistryManagementConfiguration {
	config := &baasapi.RegistryManagementConfiguration{
		Type: registry.Type,
