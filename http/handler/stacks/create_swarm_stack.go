...
func (payload *swarmStackFromFileContentPayload) Validate(r *http.Request) error {
	if govalidator.IsNull(payload.Name) {
		return baasapi.Error("Invalid stack name")
	}
	if govalidator.IsNull(payload.SwarmID) {
		return baasapi.Error("Invalid Swarm ID")
	}
	if govalidator.IsNull(payload.StackFileContent) {
		return baasapi.Error("Invalid stack file content")
	}
	return nil
}

func (handler *Handler) createSwarmStackFromFileContent(w http.ResponseWriter, r *http.Request, endpoint *baasapi.Endpoint) *httperror.HandlerError {
	...
	for _, stack := range stacks {
		if strings.EqualFold(stack.Name, payload.Name) {
			return &httperror.HandlerError{http.StatusConflict, "A stack with this name already exists", baasapi.ErrStackAlreadyExists}
		}
	}

	stackID := handler.StackService.GetNextIdentifier()
	stack := &baasapi.Stack{
		ID:         baasapi.StackID(stackID),
		Name:       payload.Name,
...
func (payload *swarmStackFromGitRepositoryPayload) Validate(r *http.Request) error {
	...
	if govalidator.IsNull(payload.RepositoryURL) || !govalidator.IsURL(payload.RepositoryURL) {
		return baasapi.Error("Invalid repository URL. Must correspond to a valid URL format")
	}
	if payload.RepositoryAuthentication && (govalidator.IsNull(payload.RepositoryUsername) || govalidator.IsNull(payload.RepositoryPassword)) {
		return baasapi.Error("Invalid repository credentials. Username and password must be specified when authentication is enabled")
	}
...
func (handler *Handler) createSwarmStackFromGitRepository(w http.ResponseWriter, r *http.Request, endpoint *baasapi.Endpoint) *httperror.HandlerError {
...
func (payload *swarmStackFromFileUploadPayload) Validate(r *http.Request) error {
	...
	if err != nil {
		return baasapi.Error("Invalid Compose file. Ensure that the Compose file is uploaded correctly")
	}
	payload.StackFileContent = composeFileContent

	var env []baasapi.Pair
	err = request.RetrieveMultiPartFormJSONValue(r, "Env", &env, true)
	if err != nil {
		return baasapi.Error("Invalid Env parameter")
	}
	payload.Env = env
	return nil
}

func (handler *Handler) createSwarmStackFromFileUpload(w http.ResponseWriter, r *http.Request, endpoint *baasapi.Endpoint) *httperror.HandlerError {
...
type swarmStackDeploymentConfig struct {
	...
	dockerhub  *baasapi.DockerHub
	registries []baasapi.Registry
	prune      bool
}

func (handler *Handler) createSwarmDeployConfig(r *http.Request, stack *baasapi.Stack, endpoint *baasapi.Endpoint, prune bool) (*swarmStackDeploymentConfig, *httperror.HandlerError) {
