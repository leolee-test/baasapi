...
func getResponseAsJSONArray(response *http.Response) ([]interface{}, error) {
	...
	case map[string]interface{}:
		if responseObject["message"] != nil {
			return nil, baasapi.Error(responseObject["message"].(string))
		}
...
func writeAccessDeniedResponse() (*http.Response, error) {
	...
	err := rewriteResponse(response, baasapi.ErrResourceAccessDenied, http.StatusForbidden)
	return response, err
}

func rewriteAccessDeniedResponse(response *http.Response) error {
	return rewriteResponse(response, baasapi.ErrResourceAccessDenied, http.StatusForbidden)
}
