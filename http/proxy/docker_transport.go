...
type (
	...
	restrictedOperationContext struct {
		...
		userTeamIDs      []baasapi.TeamID
		resourceControls []baasapi.ResourceControl
	}
	registryAccessContext struct {
		isAdmin         bool
		userID          baasapi.UserID
		teamMemberships []baasapi.TeamMembership
...
func (p *proxyTransport) proxyDockerRequest(request *http.Request) (*http.Response, error) {
	...
	if p.enableSignature {
		signature, err := p.SignatureService.CreateSignature(baasapi.BaaSapiAgentSignatureMessage)
		if err != nil {
			return nil, err
		}

		request.Header.Set(baasapi.BaaSapiAgentPublicKeyHeader, p.SignatureService.EncodedPublicKey())
		request.Header.Set(baasapi.BaaSapiAgentSignatureHeader, signature)
	}
