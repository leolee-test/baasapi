...
module.exports = function(grunt) {
  ...
  grunt.registerTask('build-server2', [
	      'shell:buildBinary:linux:' + arch,
	      'copy:templates'
	    ]);  
  ...
  grunt.registerTask('run-dev2', ['build2']);
...
function shell_buildBinary(p, a) {
  var binfile = 'dist/baasapi';
  if (p === 'linux') {
    return [
      'if [ -f ' + (binfile) + ' ]; then',
      'echo "BaaSapi binary exists";',
      'else',
    ...
    return [
      ...
      'Write-Host "BaaSapi binary exists"',
      '} else {',
...
function shell_run() {
  return [
    'docker rm -f baasapi',
    'docker run -d -p 9000:9000 -v $(pwd)/dist:/app -v /tmp/baasapi:/data -v /var/run/docker.sock:/var/run/docker.sock:z --name baasapi portainer/base /app/baasapi --no-analytics --template-file /app/templates.json'
  ].join(';');
}
