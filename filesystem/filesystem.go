...
import (
	...
	"github.com/baasapi/baasapi/api/archive"
...
func NewService(dataStorePath, fileStorePath string) (*Service, error) {
	...
	err = service.CreateDirectoryInStore(BaaSDeploymentPath)
	if err != nil {
		return nil, err
	}
...
func (service *Service) StoreYamlFileFromJSON(stackIdentifier, fileName string, content interface{}, creator string) (string, error) {
	//stackStorePath := path.Join(BaaSDeploymentPath, creator, stackIdentifier)
	stackStorePath := path.Join(BaaSDeploymentPath, stackIdentifier)
	err := service.CreateDirectoryInStore(stackStorePath)
	if err != nil {
		return "", err
	}
	...
	err = service.WriteJSONToFile(yamlFilePath, content)
	if err != nil {
		return "", err
	}
...
func (service *Service) StoreTLSFileFromBytes(folder string, fileType baasapi.TLSFileType, data []byte) (string, error) {
	...
	var fileName string
	switch fileType {
	case baasapi.TLSFileCA:
		fileName = TLSCACertFile
	case baasapi.TLSFileCert:
		fileName = TLSCertFile
	case baasapi.TLSFileKey:
		fileName = TLSKeyFile
	default:
		return "", baasapi.ErrUndefinedTLSFileType
	}
...
// StoreKubeconfigFileFromBytes creates a folder in the TLSStorePath and stores a new file from bytes.
// It returns the path to the newly created file.
func (service *Service) StoreKubeconfigFileFromBytes(folder string, fileType baasapi.TLSFileType, data []byte) (string, error) {
	storePath := path.Join(BaaSConfigPath, folder)
	//err := service.CreateDirectoryInStore(storePath)
	//if err != nil {
	//	return "", err
	//}
	...
	configFilePath := path.Join(storePath, fileName)
	r := bytes.NewReader(data)
	err := service.createFileInStore(configFilePath, r)
	if err != nil {
		return "", err
	}
	return path.Join(service.fileStorePath, configFilePath), nil
}
