...
const (
	errInvalidResponseStatus = baasapi.Error("Invalid response status (expecting 200)")
...
func (client *HTTPClient) ExecuteAzureAuthenticationRequest(credentials *baasapi.AzureCredentials) (*AzureAuthenticationResponse, error) {
	...
	if response.StatusCode != http.StatusOK {
		return nil, baasapi.ErrAzureInvalidCredentials
	}
...
func pingOperation(client *http.Client, target string) (bool, error) {
	...
	if response.Header.Get(baasapi.BaaSapiAgentHeader) != "" {
