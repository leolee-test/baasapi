...
type anchorauthenticatePayload struct {
...
func (payload *anchorauthenticatePayload) Validate(r *http.Request) error {
		return nil;
}


// GET request on /api/baask8ss
func (handler *Handler) baask8sChannelsAnchor(w http.ResponseWriter, r *http.Request) *httperror.HandlerError {
	...
	var payload anchorauthenticatePayload
	...
	if err == baasapi.ErrObjectNotFound {
		...
		req, err := http.NewRequest("POST", sdk_url+"/channels/"+channelname+"/anchorpeers", bytes.NewBuffer(jsonValue))
