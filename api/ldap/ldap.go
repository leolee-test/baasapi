...
func searchUser(username string, conn *ldap.Conn, settings []baasapi.LDAPSearchSettings) (string, error) {
	var userDN string
	found := false
...
func createConnection(settings *baasapi.LDAPSettings) (*ldap.Conn, error) {
...
func (*Service) AuthenticateUser(username, password string, settings *baasapi.LDAPSettings) error {
	...
	if err != nil {
		return baasapi.ErrUnauthorized
	}
...
func getGroups(userDN string, conn *ldap.Conn, settings []baasapi.LDAPGroupSearchSettings) []string {
