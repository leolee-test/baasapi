...
func (service *Service) Tags() ([]baasapi.Tag, error) {
	var tags = make([]baasapi.Tag, 0)
...
func (service *Service) CreateTag(tag *baasapi.Tag) error {
	return service.db.Update(func(tx *bolt.Tx) error {
		bucket := tx.Bucket([]byte(BucketName))

		id, _ := bucket.NextSequence()
		tag.ID = baasapi.TagID(id)
...
func (service *Service) DeleteTag(ID baasapi.TagID) error {
